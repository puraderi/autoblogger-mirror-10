...
Multi-tenant blog platform. Single Next.js deployment serves unlimited websites via hostname-based routing. All content AI-generated (Claude Sonnet 4.5), Swedish-localized.
...
- Lean dependencies (no AI SDKs in main app)
...
**New Website:**
1. Create database record in `website_data`
2. Visit hostname (add to `/etc/hosts` if local)
3. Check homepage renders with correct colors
4. Navigate to `/blogg`, `/om-oss`, `/kontakt`
5. Verify header/footer templates match database values

**Blog Posts:**
1. Create blog post in `blog_post` table
2. Visit `/blogg/[slug]` to see post
3. Check image loaded correctly
4. Verify tags, excerpt, metadata
