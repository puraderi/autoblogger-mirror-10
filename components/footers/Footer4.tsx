...
export default function Footer4({ websiteData }: FooterProps) {
  ...
  return (
    <footer className="mt-auto">
      ...
      {/* Main footer */}
      <div className="py-10 border-t" style={{ borderColor: websiteData.secondary_color }}>
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Left: Brand + social */}
            <div>
              <h4 className="font-bold text-lg mb-3" style={{ color: websiteData.primary_color }}>
                {websiteData.website_name}
              </h4>
              <p className="text-sm text-gray-600 mb-4 max-w-sm">{websiteData.topic}</p>
              <div className="flex items-center gap-3">
                {websiteData.social_twitter && (
                  <a href={websiteData.social_twitter} className="text-gray-400 hover:text-gray-600 transition-colors">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                  </a>
                )}
                {websiteData.social_facebook && (
                  <a href={websiteData.social_facebook} className="text-gray-400 hover:text-gray-600 transition-colors">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                  </a>
                )}
                {websiteData.social_instagram && (
                  <a href={websiteData.social_instagram} className="text-gray-400 hover:text-gray-600 transition-colors">
                ...
                {websiteData.social_linkedin && (
                  <a href={websiteData.social_linkedin} className="text-gray-400 hover:text-gray-600 transition-colors">
